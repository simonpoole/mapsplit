...
public interface OsmMap {
	...
	/**
	 * returns an entry of the hashmap at the given key or 0 if not found. The
	 * return value is encoded in an internal format and has to be parsed with
	...
	/**
	 * update key so it knows about all tiles where it is needed. The given
	 * longs in the list are expected in this map's format. Neighbourhood
	 * of the tile-IDs are evaluated.
	 * @param key
	 * @param tiles
	 */
	 ...
	 * base-tile, neighbours and tiles where this key is connected to
	 * other keys, e.g. by a way. The format of the integer is 
	 * (tileX << 13 | tileY).
	...
	/**
	 * for debugging this method tells you how much of the buckets are
	 * used. A load < 0,5 is desirable for good speed
